...
class ControllerExtensionDashboardCustomers extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('dashboard_customers', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/dashboard/customers', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/dashboard/customers', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=dashboard', true);

		if (isset($this->request->post['dashboard_customers_width'])) {
			$data['dashboard_customers_width'] = $this->request->post['dashboard_customers_width'];
		} else {
			$data['dashboard_customers_width'] = $this->config->get('dashboard_customers_width');
		}
		...
		if (isset($this->request->post['dashboard_customers_status'])) {
			$data['dashboard_customers_status'] = $this->request->post['dashboard_customers_status'];
		} else {
			$data['dashboard_customers_status'] = $this->config->get('dashboard_customers_status');
		}

		if (isset($this->request->post['dashboard_customers_sort_order'])) {
			$data['dashboard_customers_sort_order'] = $this->request->post['dashboard_customers_sort_order'];
		} else {
			$data['dashboard_customers_sort_order'] = $this->config->get('dashboard_customers_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('extension/dashboard/customers_form', $data));
	}
