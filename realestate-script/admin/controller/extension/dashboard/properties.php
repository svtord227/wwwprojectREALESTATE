...
class ControllerExtensionDashboardProperties extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('dashboard_properties', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/dashboard/properties', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/dashboard/properties', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=dashboard', true);

		if (isset($this->request->post['dashboard_properties_width'])) {
			$data['dashboard_properties_width'] = $this->request->post['dashboard_properties_width'];
		} else {
			$data['dashboard_properties_width'] = $this->config->get('dashboard_properties_width');
		}
		...
		if (isset($this->request->post['dashboard_properties_status'])) {
			$data['dashboard_properties_status'] = $this->request->post['dashboard_properties_status'];
		} else {
			$data['dashboard_properties_status'] = $this->config->get('dashboard_properties_status');
		}

		if (isset($this->request->post['dashboard_properties_sort_order'])) {
			$data['dashboard_properties_sort_order'] = $this->request->post['dashboard_properties_sort_order'];
		} else {
			$data['dashboard_properties_sort_order'] = $this->config->get('dashboard_properties_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('extension/dashboard/properties_form', $data));
	}
