...
class ControllerAgentForgotten extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_agent_agent->editCode($this->request->post['email'], $code);
			...
			if ($this->config->get('config_customer_activity')) {
				...
				if ($agent_info) {
					...
					$activity_data = array(
						'property_agent_id' => $agent_info['property_agent_id'],
						'name'        => $agent_info['firstname'] . ' ' . $agent_info['lastname']
					);

					$this->model_agent_activity->addActivity('forgotten', $activity_data);
				}
			}
